...
void ImportationPeriodicallyEvent::schedule_event(
    Scheduler* scheduler, const int& location, const int& duration,
...
void ImportationPeriodicallyEvent::execute() {
  ...
  schedule_event(
      Model::SCHEDULER, location_, duration_, genotype_id_, number_of_cases_,
  ...
  for (auto i = 0; i < number_of_importation_cases; i++) {
    ...
    uint32_t random_id = Model::RANDOM->random_uniform_int(
        0,
        static_cast<const uint32_t &>(Model::CONFIG
