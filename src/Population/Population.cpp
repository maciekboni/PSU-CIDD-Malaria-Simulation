  ...
  for (auto state = 0; state < Person::NUMBER_OF_STATE - 1; state++) {
    for (auto ac = 0; ac < Model::CONFIG->number_of_age_classes(); ac++) {
      for (auto i = 0; i < pi_lsa->vPerson()[location][state][ac].size(); i++) {
        if (pi_lsa->vPerson()[location][state][ac][i]->residence_location()
            == location) {
          temp++;
        }
      }
  ...
  for (auto loc = 0; loc < Model::CONFIG->number_of_locations(); loc++) {
    for (auto parasite_type_id = 0;
      ...
      const auto new_beta = Model::CONFIG->location_db()[loc].beta
                            * Model::MODEL->get_seasonal_factor(
      ...
      //             if (Global::scheduler->currentTime >=
      //             Global::startTreatmentDay) {
      //                //scale down to 1000
      ...
      for (auto bitting_level = 0; bitting_level < v_int_number_of_bites.size();
           bitting_level++) {
        ...
        for (auto j = 0u; j < v_int_number_of_bites[bitting_level]; j++) {
          ...
          if (Model::CONFIG
                  ->using_variable_probability_infectious_bites_cause_infection()) {
          ...
          } else if (p_infectious
                     <= Model::CONFIG->p_infection_from_an_infectious_bite()) {
            if (person->host_state() != Person::EXPOSED
                && person->liver_parasite_type() == nullptr) {
              person->today_infections()->push_back(parasite_type_id);
              today_infections.push_back(person);
  ...
  for (auto* p : today_infections) {
    if (!p->today_infections()->empty()) {
      Model::DATA_COLLECTOR
          ->monthly_number_of_new_infections_by_location()[p->location()] += 1;
    }
...
void Population::initialize() {
  if (model() != nullptr) {
    // get population size, number of location, age_distribution from
    // Model::CONFIG
    ...
    for (auto loc = 0; loc < number_of_location; loc++) {
      const auto popsize_by_location = static_cast<int>(
          Model::CONFIG->location_db()[loc].population_size
      ...
      for (auto age_class = 0;
        ...
        //                std::cout << loc << "\t" << age_class << "\t" <<
        //                number_of_individual_by_loc_ageclass << std::endl;
        for (auto i = 0; i < number_of_individual_by_loc_ageclass; i++) {
          ...
          // std::cout << i << "\t" << age_class << "\t" << age_from << "\t" <<
          // age_to << std::endl;

          // set age will also set ageclass
          p->set_age(static_cast<const int&>(
              Model::RANDOM->random_uniform_int(age_from, age_to + 1)));

          int days_to_next_birthday =
              Model::RANDOM->random_uniform(Constants::DAYS_IN_YEAR());
          // std::cout << days_to_next_birthday << std::endl;
          auto simulation_time_birthday =
              TimeHelpers::get_simulation_time_birthday(
                  days_to_next_birthday, p->age(),
                  Model::SCHEDULER->calendar_date);
          p->set_birthday(simulation_time_birthday);

          LOG_IF(simulation_time_birthday > 0, FATAL)
              << "simulation_time_birthday have to be <= 0 when initilizing "
                 "population";
          ...
          if (simulation_time_birthday + Constants::DAYS_IN_YEAR() / 2 >= 0) {
            ...
            // LOG(INFO) << "Infant: " << p->age() << " - " <<
            // simulation_time_birthday;
            p->immune_system()->set_immune_component(
                new InfantImmuneComponent());
            // schedule for switch
            SwitchImmuneComponentEvent::
                schedule_for_switch_immune_component_event(
                    Model::SCHEDULER, p,
                    simulation_time_birthday + Constants::DAYS_IN_YEAR() / 2);
          } else {
            // LOG(INFO) << "Adult: " << p->age() << " - " <<
            // simulation_time_birthday;
            p->immune_system()->set_immune_component(
                new NonInfantImmuneComponent());
          }
          ...
          p->immune_system()->immune_component()->set_latest_value(
              immune_value);
          ...
          p->set_base_bitting_level_value(
              Model::CONFIG->relative_bitting_info()
                  .v_biting_level_value[p->bitting_level()]);
          //                    p->update_bitting_level();

          p->set_moving_level(
              Model::CONFIG->moving_level_generator().draw_random_level(
                  Model::RANDOM));

          p->set_latest_update_time(0);

          int time =
              Model::RANDOM->random_uniform(Model::CONFIG->update_frequency())
              + 1;
...
void Population::introduce_initial_cases() {
  if (model_ != nullptr) {
    ...
    for (const auto p_info : Model::CONFIG->initial_parasite_info()) {
      ...
      auto* genotype =
          Model::CONFIG->genotype_db()->at(p_info.parasite_type_id);
      LOG(INFO) << "Introducing genotype " << p_info.parasite_type_id
                << " with prevalence: " << p_info.prevalence << " : "
                << num_of_infections << " infections at location "
                << p_info.location;
      // std::cout << p_info.location << "-" << p_info.parasite_type_id << "-"
      // << num_of_infections << std::endl;
...
void Population::introduce_parasite(const int& location,
  ...
  if (model_ != nullptr) {
    ...
    model_->random()->random_multinomial(vLevelDensity.size(),
                                         num_of_infections, &vLevelDensity[0],
                                         &vIntNumberOfBites[0]);

    for (auto biting_level = 0; biting_level < vIntNumberOfBites.size();
         biting_level++) {
      const int size = pi->vPerson()[location][biting_level].size();
      if (size == 0) continue;
      for (auto j = 0u; j < vIntNumberOfBites[biting_level]; j++) {
        //                std::cout << vIntNumberOfBites[bitting_level] << "-"
        //                << j << std::endl;
        // select 1 random person from level i
...
void Population::update_force_of_infection(const int& current_time) {
  ...
  for (auto loc = 0; loc < Model::CONFIG->number_of_locations(); loc++) {
    for (auto p_type = 0; p_type < Model::CONFIG->number_of_parasite_types();
         p_type++) {
...
void Population::perform_birth_event() {
  ...
  for (auto loc = 0; loc < Model::CONFIG->number_of_locations(); loc++) {
    auto poisson_means =
        size(loc) * Model::CONFIG->birth_rate() / Constants::DAYS_IN_YEAR();
...
void Population::give_1_birth(const int& location) {
  ...
  //    p->startLivingTime = (Global::startTreatmentDay >
  //    Global::scheduler->currentTime) ? Global::startTreatmentDay :
  ...
  for (auto loc = 0; loc < Model::CONFIG->number_of_locations(); loc++) {
    for (auto hs = 0; hs < Person::NUMBER_OF_STATE - 1; hs++) {
      ...
      for (auto ac = 0; ac < Model::CONFIG->number_of_age_classes(); ac++) {
        ...
        if (size == 0) continue;
        auto poisson_means = size * Model::CONFIG->death_rate_by_age_class()[ac]
        ...
        for (int i = 0; i < number_of_deaths; i++) {
          ...
          //                    std::cout << index << "-" <<
          //                    pi->vPerson()[loc][hs][ac].size() << std::endl;
...
void Population::clear_all_dead_state_individual() {
  // return all Death to object pool and clear vPersonIndex[l][dead][ac] for all
  // location and ac
  auto pi = get_person_index<PersonIndexByLocationStateAgeClass>();
...
void Population::perform_circulation_event() {
  ...
  std::vector<int> v_number_of_residents_by_location(
      Model::CONFIG->number_of_locations(), 0);

  for (auto location = 0; location < Model::CONFIG->number_of_locations();
       location++) {
    //        v_number_of_residents_by_location[target_location] =
    //        (size(target_location));
    v_number_of_residents_by_location[location] =
        Model::DATA_COLLECTOR->popsize_residence_by_location()[location];
    //        std::cout << v_original_pop_size_by_location[target_location] <<
    //        std::endl;
  }

  for (int from_location = 0;
    ...
    for (int target_location = 0;
         target_location < Model::CONFIG->number_of_locations();
         target_location++) {
      //            std::cout << v_num_leavers_to_destination[target_location]
      //            << std::endl;
      ...
      perform_circulation_for_1_location(
          from_location, target_location,
          v_num_leavers_to_destination[target_location], today_circulations);
    }
  }
...
void Population::perform_circulation_for_1_location(
  ...
  for (int moving_level = 0; moving_level < vIntNumberOfCirculation.size();
       moving_level++) {
    auto size =
...
void Population::perform_interupted_feeding_recombination() {
  ...
  if (sum_z > 0.0001) {
      ...
      for (auto i = 0; i < Model::CONFIG->number_of_parasite_types(); i++) {
        ...
        for (auto j = 0; j < Model::CONFIG->number_of_parasite_types(); j++) {
          ...
          if (i == j) {
            ...
            for (auto p = 0; p < Model::CONFIG->number_of_parasite_types();
                 p++) {
    ...
    for (auto loc = 0; loc < Model::CONFIG->number_of_locations(); loc++) {
      auto new_z = std::vector<unsigned int>(
          Model::CONFIG->number_of_parasite_types(), 0);
      Model::RANDOM->random_multinomial(
