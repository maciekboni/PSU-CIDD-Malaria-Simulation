...
void Person::set_host_state(const HostStates& value) {
  if (host_state_ != value) {
    ...
    if (value == DEAD) {
      ...
      //
      //            Model::STATISTIC->update_person_days_by_years(location_,
      //            -(Constants::DAYS_IN_YEAR() -
      //            Model::SCHEDULER->current_day_in_year()));
      Model::DATA_COLLECTOR->record_1_death(
          location_, birthday_, number_of_times_bitten_, age_class_, age_);
    }
...
void Person::notify_change_in_force_of_infection(
  ...
  //    double weight = pow(10, blood_parasite_log_relative_density -
  //    log_total_relative_parasite_density); assert(weight <=1 && weight >=0);
...
double Person::get_biting_level_value() {
  return Model::CONFIG->relative_bitting_info()
      .v_biting_level_value[bitting_level_];
}
...
//
//     for(Event* e :  *events()) {
//         if (dynamic_cast<TestTreatmentFailureEvent*> (e) != nullptr &&
//         e->executable()) {
//             //            e->set_dispatcher(nullptr);
//             //record treatment failure
//             Model::DATA_COLLECTOR->record_1_treatment_failure_by_therapy(location_,
//             age_, ((TestTreatmentFailureEvent*) e)->therapyId());
//
//         }
//     }
// }
...
void Person::schedule_end_clinical_due_to_drug_resistance_event(
    ClonalParasitePopulation* blood_parasite) {
  auto d_clinical = Model::RANDOM->random_normal(7, 2);
  d_clinical = std::min<int>(std::max<int>(d_clinical, 5), 14);

  EndClinicalDueToDrugResistanceEvent::schedule_event(
      Model::SCHEDULER, this, blood_parasite,
...
int Person::complied_dosing_days(const int& dosing_day) const {
  if (Model::CONFIG->p_compliance() < 1) {
    ...
    if (p > Model::CONFIG->p_compliance()) {
      // do not comply
      const auto a = (Model::CONFIG->min_dosing_days() - dosing_day)
                     / (1 - Model::CONFIG->p_compliance());
      return static_cast<int>(
          std::ceil(a * p + Model::CONFIG->min_dosing_days() - a));
    }
  }
  return dosing_day;
}
...
void Person::receive_therapy(Therapy* therapy,
  ...
  if (sc_therapy != nullptr) {
    for (int j = 0; j < sc_therapy->drug_ids.size(); ++j) {
      auto dosing_days =
          sc_therapy->drug_ids.size() == sc_therapy->dosing_day.size()
              ? sc_therapy->dosing_day[j]
    ...
    for (auto i = 0; i < mac_therapy->therapy_ids().size(); i++) {
      ...
      if (start_day == 1) {
        receive_therapy(Model::CONFIG->therapy_db()[therapy_id],
...
void Person::add_drug_to_blood(DrugType* dt, const int& dosing_days,
  ...
  if (is_part_of_MAC_therapy) {
    ...
    } else if (starting_drug_values_for_MAC.find(dt->id())
               != starting_drug_values_for_MAC.end()) {
...
void Person::determine_clinical_or_not(
  ...
  if (all_clonal_parasite_populations_->contain(clinical_caused_parasite)) {
    ...
    //        if (P <= Model::CONFIG->p_relapse()) {
    //    if (Model::SCHEDULER->current_time() >= 2000 &&
    //    Model::SCHEDULER->current_time() <= 2010)
    //      std::cout << this->age() << "\t" <<
    //      this->immune_system()->get_current_value() << "\t"
...
void Person::update_bitting_level() {
  if (Model::CONFIG->using_age_dependent_bitting_level()) {
    // TODO: test here
    const auto new_bitting_level_value =
        base_bitting_level_value_ * get_age_dependent_biting_factor();
    const auto diff_in_level = static_cast<int>(
        std::floor(new_bitting_level_value - get_biting_level_value())
        / ((Model::CONFIG->relative_bitting_info().max_relative_biting_value
           ...
           / static_cast<double>(
               ...
               - 1)));
    if (diff_in_level != 0) {
      //            std::cout << bitting_level_ << "\t" << diff_in_level <<
      //            std::endl;
