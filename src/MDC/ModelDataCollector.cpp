...
void ModelDataCollector::initialize() {
  if (model_ != nullptr) {
    ...
    fraction_of_positive_that_are_clinical_by_location_ =
        DoubleVector(Model::CONFIG->number_of_locations(), 0.0);
    popsize_by_location_hoststate_ =
        IntVector2(Model::CONFIG->number_of_locations(), IntVector(Person::NUMBER_OF_STATE, 0));
    popsize_by_location_age_class_ = IntVector2(
        Model::CONFIG->number_of_locations(), IntVector(Model::CONFIG->number_of_age_classes(), 0));
    ...
    cumulative_clinical_episodes_by_location_age_ =
        LongVector2(Model::CONFIG->number_of_locations(), LongVector(100, 0));
    cumulative_clinical_episodes_by_location_age_group_ =
        LongVector2(Model::CONFIG->number_of_locations(),
                    LongVector(Model::CONFIG->number_of_age_classes(), 0));
    ...
    cumulative_number_treatments_by_location_ =
        LongVector(Model::CONFIG->number_of_locations(), 0.0);
    ...
    total_number_of_treatments_60_by_location_ = IntVector2(
        Model::CONFIG->number_of_locations(), IntVector(Model::CONFIG->tf_window_size(), 0));
    ...
    number_of_treatments_success_with_therapy_ID_ =
    ...
    multiple_of_infection_by_location_ =
        IntVector2(Model::CONFIG->number_of_locations(), IntVector(number_of_reported_MOI, 0));

    current_EIR_by_location_ = DoubleVector(Model::CONFIG->number_of_locations(), 0.0);
    last_update_total_number_of_bites_by_location_ =
        LongVector(Model::CONFIG->number_of_locations(), 0);

    last_10_blood_slide_prevalence_by_location_ =
        DoubleVector2(Model::CONFIG->number_of_locations(), DoubleVector(10, 0.0));
    ...
    number_of_deaths_by_location_age_year_ =
        IntVector2(Model::CONFIG->number_of_locations(), IntVector(80, 0));
    number_of_malaria_deaths_by_location_age_year_ =
        IntVector2(Model::CONFIG->number_of_locations(), IntVector(80, 0));
    ...
    total_number_of_treatments_60_by_therapy_ = IntVector2(
        Model::CONFIG->therapy_db().size(), IntVector(Model::CONFIG->tf_window_size(), 0));
    ...
    monthly_number_of_new_infections_by_location_ =
        IntVector(Model::CONFIG->number_of_locations(), 0);
...
void ModelDataCollector::perform_population_statistic() {
  ...
  //     blood_slide_prevalence_by_location_.assign(Model::CONFIG->number_of_locations(),
  //     0.0);
  //
  //     popsize_by_location_hoststate_.clear();
  //     popsize_by_location_hoststate_.assign(Model::CONFIG->number_of_locations(),
  //     IntVector(Person::NUMBER_OF_STATE, 0));
  //
  //     total_immune_by_location_.clear();
  //     total_immune_by_location_.assign(Model::CONFIG->number_of_locations(),
  //     0.0);
  //
  //     total_immune_by_location_age_class_.clear();
  //     total_immune_by_location_age_class_.assign(Model::CONFIG->number_of_locations(),
  //     DoubleVector(Model::CONFIG->number_of_age_classes(), 0.0));
  ...
  for (auto loc = 0ul; loc < Model::CONFIG->number_of_locations(); loc++) {
        ...
        for (int i = 0; i < size; i++) {
          ...
          if (moi > 0) {
            ...
            if (moi > number_of_reported_MOI) {
              //                            multiple_of_infection_by_location_[loc][number_of_reported_MOI
              //                            - 1]++;
            } else {
    ...
    //        double number_of_assymptomatic_and_clinical =
    ...
    blood_slide_prevalence_by_location_[loc] =
        blood_slide_prevalence_by_location_[loc] / static_cast<double>(pop_sum_location);

    current_EIR_by_location_[loc] = (total_number_of_bites_by_location_[loc]
    ...
    last_10_blood_slide_prevalence_by_location_[loc][(Model::SCHEDULER->current_time()
                                                      / Model::CONFIG->report_frequency())
                                                     % 10] =
        blood_slide_prevalence_by_location_[loc];
    last_10_fraction_positive_that_are_clinical_by_location_
        [loc][(Model::SCHEDULER->current_time() / Model::CONFIG->report_frequency()) % 10] =
            fraction_of_positive_that_are_clinical_by_location_[loc];

    for (int ac = 0; ac < Model::CONFIG->number_of_age_classes(); ac++) {
      last_10_fraction_positive_that_are_clinical_by_location_age_class_
          [loc][ac][(Model::SCHEDULER->current_time() / Model::CONFIG->report_frequency()) % 10] =
  ...
  for (auto loc = 0; loc < Model::CONFIG->number_of_locations(); loc++) {
    if (EIR_by_location_year_[loc].empty()) {
      ...
      EIR_by_location_[loc] = ((EIR_by_location_year_[loc].size() - number_of_0) == 0.0)
                                  ? 0.0
                                  : sum_eir / (EIR_by_location_year_[loc].size() - number_of_0);
    }
  }
}
...
void ModelDataCollector::record_1_death(const int& location, const int& birthday,
  ...
  if (Model::SCHEDULER->current_time() >= Model::CONFIG->start_collect_data_day()) {
    update_person_days_by_years(
        location, -(Constants::DAYS_IN_YEAR() - Model::SCHEDULER->current_day_in_year()));
...
void ModelDataCollector::update_average_number_bitten(const int& location, const int& birthday,
  ...
  const auto time_living_from_start_collect_data_day =
      (birthday < Model::CONFIG->start_collect_data_day())
          ? 1
          : Model::SCHEDULER->current_time() + 1 - Model::CONFIG->start_collect_data_day();
...
void ModelDataCollector::begin_time_step() {
  ...
  for (auto therapy_id = 0; static_cast<size_t>(therapy_id) < Model::CONFIG->therapy_db().size();
       therapy_id++) {
...
void ModelDataCollector::end_of_time_step() {
  if (Model::SCHEDULER->current_time() >= Model::CONFIG->start_collect_data_day()) {
    ...
    for (auto location = 0; location < Model::CONFIG->number_of_locations(); location++) {
      ...
      total_TF_60_by_location_[location]
                              [Model::SCHEDULER->current_time() % Model::CONFIG->tf_window_size()] =
    ...
    // update UTL
    if ((avg_tf / static_cast<double>(Model::CONFIG->number_of_locations()))
        <= Model::CONFIG->tf_rate()) {
    ...
    for (auto therapy_id = 0; static_cast<size_t>(therapy_id) < Model::CONFIG->therapy_db().size();
      ...
      total_tf_60_by_therapy_[therapy_id]
                             [Model::SCHEDULER->current_time() % Model::CONFIG->tf_window_size()] =
...
// void ModelDataCollector::collect_1_non_resistant_treatment(const int&
// therapy_id) {
//     number_of_treatments_with_therapy_ID_[therapy_id] += 1;
// }
...
void ModelDataCollector::record_1_TF(const int& location, const bool& by_drug) {
  ...
  if (Model::SCHEDULER->current_time() >= Model::CONFIG->start_of_comparison_period()) {
    const auto current_discounted_tf =
        exp(log(0.97)
            * floor((Model::SCHEDULER->current_time() - Model::CONFIG->start_collect_data_day())
                    / Constants::DAYS_IN_YEAR()));
...
void ModelDataCollector::record_1_treatment_failure_by_therapy(const int& location, const int& age,
                                                               const int& therapy_id) {
