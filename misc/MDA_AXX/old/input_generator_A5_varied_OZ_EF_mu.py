                    ...
                    for oz_eff, ec50_dict in oz_efficacies.items():   
                        for strategy in mda_stategy:
                            for mu_factor in mu_factors:
                                ...
                                for index,event in enumerate(data['events']):
                                    if itc == '' and event['name'] == 'change_treatment_coverage':
                                        new_data['events'][index]['info']= []                                            
                                    
                                    if imp == '' and event['name'] == 'introduce_parasites_periodically':
                                        new_data['events'][index]['info']= []
                                    
                                    if event['name'] == 'single_round_MDA':
                                        new_data['events'][index]['info'] = data['events'][index]['info'][0:mda_round]
                                    
                                    if event['name'] == 'turn_on_mutation':
                                        new_data['events'][index]['info'][0]['mutation_probability'] = data['events'][index]['info'][0]['mutation_probability'] * mu_factor 
                                ...
                                output_filename = 'A5_varied_OZEF_mu/ONELOC_40k_%dRMDA_%s_OPPUNIFORM_FLAL_%s_%s%s%s_OZFQ_EF_%d_mu_factor_%d.yml'%(
                                        mda_round,pfpr_str,strategy, mda_therapy[1], itc,imp,oz_eff, mu_factor);
