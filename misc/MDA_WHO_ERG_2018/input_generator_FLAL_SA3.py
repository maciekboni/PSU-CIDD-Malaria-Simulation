...
#    sample_sizes = 50000;
#    paramspace = lhs(6, samples= sample_sizes)
#    
...
#    
#    paramspace[:, 5] *= (mda_coverages[1] - mda_coverages[0])
#    paramspace[:, 5] += mda_coverages[0]
#    
#    df = pd.DataFrame(data=paramspace, columns=["popsize", "beta","importation","itc","cr_factor","mda_coverage"]);
#    
#    # shufle data frame 
#    df = df.sample(frac=1).reset_index(drop=True)
#    
#    df.to_csv("SA3_paramspace_%d.csv"%mda_round,index=False);
#%%
...
for mda_round in number_MDA_round:
    df = pd.read_csv("SA3_paramspace_%d.csv"%mda_round);
    for r_index in range(run_index_from, run_index_to):
        ...
        new_data['mean_prob_individual_present_at_mda'] = [row.mda_coverage.item()+0.05, row.mda_coverage.item()-0.05, row.mda_coverage.item()+0.05]
                    
        output_filename = 'FLAL_SA3/%d/%d.yml'%(mda_round, r_index);
