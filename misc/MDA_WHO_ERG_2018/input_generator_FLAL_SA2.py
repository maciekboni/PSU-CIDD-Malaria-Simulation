...
#
#df.to_csv("SA2_paramspace.csv",index=False);
#%%
...
df = pd.read_csv("SA2_paramspace.csv");
...
for mda_round in number_MDA_round:
    for index, row in df.iterrows():
        ...
        #population size 
        data['location_db']['population_size_by_location'] = [row.popsize];
        ...
        for index,event in enumerate(data['events']):
            ...
            if event['name'] == 'introduce_parasites_periodically':
                ...
                new_data['events'][index]['info'][0]['parasite_info'][0]['number_of_cases'] = round(row.importation*10000)
            
            if event['name'] == 'change_treatment_coverage':
                if row.itc == 0:                                    
                    new_data['events'][index]['info']= []                        
                else:
                    new_data['events'][index]['info'][0]['p_treatment_for_less_than_5_by_location_to'] = np.full(number_of_locations, row.itc).tolist()
                    new_data['events'][index]['info'][0]['p_treatment_for_more_than_5_by_location_to'] = np.full(number_of_locations, row.itc).tolist()
                    
        
        for locus_index,locus in enumerate(new_data['genotype_info']['loci']):
            for allele_index, allele in enumerate(locus['alleles']):
                new_data['genotype_info']['loci'][locus_index]['alleles'][allele_index]['daily_cost_of_resistance'] = cal_new_cr(data['genotype_info']['loci'][locus_index]['alleles'][allele_index]['daily_cost_of_resistance'], row.cr_factor)
