...
stream = open('input_novel_antimalarials_3d.yml', 'r');
            ...
            for tf, tf_str in tf_switch.items():
                for rf, rf_str in replacement_fractions.items():
                    for ec50, ec50_str in KAF_EC50s.items():
                        for mu_factor, mu_str in mutation_factors.items():
                            ...
                            for index,event in enumerate(data['events']):
                                if event['name'] == 'turn_on_mutation':
                                    new_data['events'][index]['info'][0]['mutation_probability'] = 0.001983*mu_factor   
                            
                            output_filename = 'C2/NovelDrug_PFPR_%s_TC_%s_FL_%s_TF_%s_Q_%s_EC50_%s_mu_%s.yml'%( pfpr,tc_str, fl_str, tf_str, rf_str, ec50_str, mu_str);
