            ...
            for comp, comp_str in compliance.items():        
                for tc, tc_map in treatment_coverages.items():            
                    for beta, pfpr in tc_map['beta_pfpr'].items():
                        ...
                        stream = open('sd_0p20/EC50_profile_%d.yml'%(profile_i), 'r');
                        ...
                        ## save to file
                        output_filename = 'sd_0p20_inputs/%s_profile_%d_mu_0p001983_introduce_mutant_%s_comp_%s_tc_%s_pfpr_%s.yml'%( fl, profile_i ,mu_str, comp_str, tc_map['f'], pfpr);
