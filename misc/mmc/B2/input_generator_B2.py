            ...
            for comp, comp_str in compliance.items():        
                for tc, tc_map in treatment_coverages.items():            
                    for beta, pfpr in tc_map['beta_pfpr'].items():
                        ...
                        stream = open('input_B2_v%d.yml'%(profile_i), 'r');
                        ...
                        ## save to file
                        output_filename = 'B2_inputs/%s_profile_%d_mu_0p001983_introduce_mutant_%s_comp_%s_tc_%s_pfpr_%s.yml'%( fl, profile_i ,mu_str, comp_str, tc_map['f'], pfpr);
